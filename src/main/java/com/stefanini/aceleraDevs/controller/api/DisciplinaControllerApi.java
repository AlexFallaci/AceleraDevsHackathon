...
import com.stefanini.aceleraDevs.forms.DisciplinaForm;
...
public class DisciplinaControllerApi {
	...
	@Autowired
	public DisciplinaControllerApi(DisciplinaService disciplinaService, DisciplinaDTOService disciplinaDTOService,
	...
	@GetMapping
	public ResponseEntity<List<DisciplinaDTO>> listaDisciplina() {
	...
	@PostMapping
	public ResponseEntity<?> createDisciplina(@RequestBody @Validated DisciplinaDTO disciplina,
		...
		try {
			...
			URI uri = uriBuilder.path("/api/disciplina/{id}").buildAndExpand(disciplinaSaved.getId()).toUri();
	...
	@GetMapping("/{id}")
	public ResponseEntity<?> readDisciplina(@PathVariable Long id) {
		try {
